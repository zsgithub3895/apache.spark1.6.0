...
public class LagphasePlayCount {
	private static Logger logger = Logger.getLogger(LagphasePlayCount.class);
	private static HashMap<String, Long> playCount=new HashMap<String,Long>();

	@SuppressWarnings("deprecation")
	public static void main(String[] args) throws Exception {
		if (null == args || args.length < 4) {
			System.err.println("Usage: LagphasePlayCount <zkQuorum> <group> <topics> <numThreads>");
			System.exit(1);
		}
		SparkConf sparkConf = new SparkConf().setAppName("LagphasePlayCount");
		JavaSparkContext ctx = new JavaSparkContext(sparkConf);
		String lagphasePlayCount=null;
		try{
			lagphasePlayCount = PropHelper.getProperty("LAG_PHASE_PLAY_COUNT");
			logger.info("+++++++++获取发生卡顿播放次数的时间配置(分钟)："+lagphasePlayCount);
		}catch(Exception e){
			logger.info("获取发生卡顿播放次数的时间配置失败！",e);
		}
		
		// 日志以|线分隔
		final Broadcast<Pattern> V_LINE = ctx.broadcast(Pattern.compile("\\|"));
		final Broadcast<HashMap<String, Long>> playCountB = ctx.broadcast(playCount);
		if (null != lagphasePlayCount && !lagphasePlayCount.equals("")) {
			JavaStreamingContext jssc = new JavaStreamingContext(ctx,
					Durations.minutes(Long.valueOf(lagphasePlayCount)));
			...
			lines.foreachRDD(new Function2<JavaRDD<String>, Time, Void>() {
				...
				public Void call(JavaRDD<String> rdd, Time time) {
					...
					JavaRDD<LagPhaseBehaviorLog> rowRDD = rdd.map(new Function<String, LagPhaseBehaviorLog>() {
						...
						public LagPhaseBehaviorLog call(String word) {
							...
							if (StringUtils.isNotBlank(word)) {
								...
								try {
									lagLog = new LagPhaseBehaviorLog(lineArr[1], lineArr[3], lineArr[4], lineArr[5],
											lineArr[7]);
					...
					//** 维度不包含ALL *********************************************************************//*
					noALLByDimension(sqlContext, dimension,playCountB);
					//** 维度中包含ALL ******************************************************************//*
					differentALLByDimension(sqlContext, dimension,playCountB);
					return null;
				}
			});
			jssc.start();
	 ...
	 public static void noALLByDimension(SQLContext sqlContext, String[] dimension,final Broadcast<HashMap<String, Long>> hhm) {
			DataFrame noALLDimension = sqlContext.sql("select count(distinct hasID)," + dimension[0] + "," + dimension[1]
			...
			noALLDimension.toJavaRDD().map(new Function<Row, Void>() {
				...
				public Void call(Row row) {
					...
					logger.info("维度中不包含ALL的key：" + kk + "    卡顿播放次数：" + row.getLong(0));
	 ...
	 public static void differentALLByDimension(SQLContext sqlContext, String[] dimension,final Broadcast<HashMap<String, Long>> hhm) {
			...
			for (int i = 0; i < dimension.length; i++) {
				sb = new StringBuffer();
				sb.append("select count(distinct hasID)," + i + "," + dimension[i] + " from LagPhaseLog group by "
						+ dimension[i]);
			    ...
			    dAll.toJavaRDD().map(new Function<Row, Void>() {
					...
					public Void call(Row row) {
						...
						String kk = null;
						if (row.getInt(1) == 0) {
							kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#ALL#ALL#ALL#" + time;
						}
						if (row.getInt(1) == 1) {
							kk = Constant.LAG_PLAY + "#ALL#" + row.getString(2) + "#ALL#ALL#" + time;
						}
						if (row.getInt(1) == 2) {
							kk = Constant.LAG_PLAY + "#ALL#ALL#" + row.getString(2) + "#ALL#" + time;
						}
						if (row.getInt(1) == 3) {
							kk = Constant.LAG_PLAY + "#ALL#ALL#ALL#" + row.getString(2) + "#" + time;
						}
						logger.info("维度中包含三个ALL的key：" + kk + "    卡顿播放次数：" + row.getLong(0));
						hhm.getValue().put(kk, row.getLong(0));
						return null;
					}
				}).collect();
				...
				for (int j = i + 1; j < dimension.length; j++) {
				    ...
				    ddAll.toJavaRDD().map(new Function<Row, Void>() {
						...
						public Void call(Row row) {
							...
							String kk = null;
							if (row.getInt(1) == 1) {
								kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#" + row.getString(3) + "#ALL#ALL#"
										+ time;
							}
							if (row.getInt(1) == 2) {
								kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#ALL#" + row.getString(3) + "#ALL#"
										+ time;
							}
							if (row.getInt(1) == 3) {
								kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#ALL#ALL#" + row.getString(3) + "#"
										+ time;
							}
							if (row.getInt(1) == 12) {
								kk = Constant.LAG_PLAY + "#ALL#" + row.getString(2) + "#" + row.getString(3) + "#ALL#"
										+ time;
							}
							if (row.getInt(1) == 13) {
								kk = Constant.LAG_PLAY + "#ALL#" + row.getString(2) + "#ALL#" + row.getString(3) + "#"
										+ time;
							}
							if (row.getInt(1) == 23) {
								kk = Constant.LAG_PLAY + "#ALL#ALL#" + row.getString(2) + "#" + row.getString(3) + "#"
										+ time;
							}
							logger.info("维度中包含两个维度为ALL的key：" + kk + "    卡顿播放次数：" + row.getLong(0));
							hhm.getValue().put(kk, row.getLong(0));
							return null;
						}
					}).collect();
					...
					for (int k = j + 1; k < dimension.length; k++) {
						...
						dddAll.toJavaRDD().map(new Function<Row, Void>() {
							...
							public Void call(Row row) {
								...
								String kk = null;
								if (row.getInt(1) == 12) {
									kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#" + row.getString(3) + "#"
											+ row.getString(4) + "#ALL#" + time;
								}
								if (row.getInt(1) == 13) {
									kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#" + row.getString(3) + "#ALL#"
											+ row.getString(4) + "#" + time;
								}
								if (row.getInt(1) == 23) {
									kk = Constant.LAG_PLAY + "#" + row.getString(2) + "#ALL#" + row.getString(3) + "#"
											+ row.getString(4) + "#" + time;
								}
								if (row.getInt(1) == 123) {
									kk = Constant.LAG_PLAY + "#ALL#" + row.getString(2) + "#" + row.getString(3) + "#"
											+ row.getString(4) + "#" + time;
								}
								logger.info("维度中包含一个维度为ALL的key：" + kk + "    卡顿播放次数：" + row.getLong(0));
								hhm.getValue().put(kk, row.getLong(0));
								return null;
							}
						}).collect();
			...
			if (null!= hhm.getValue() && hhm.getValue().size() > 0) {
				logger.info("++++++++++++++++++++++++++++playcount中map的大小=" + hhm.getValue().size());
				/** 卡顿播放次数入Redis 和 mysql**/
				IndexToRedis.playCountToRedis(hhm.getValue());
				hhm.getValue().clear();
			}
		...
		public static void fourALLDimensions(SQLContext sqlContext,StringBuffer sb,final Broadcast<HashMap<String, Long>> hhm){
			sb =new StringBuffer();
			sb.append("select count(distinct hasID) from LagPhaseLog ");
		    ...
		    fourALLDimension.toJavaRDD().map(new Function<Row, Void>() {
				...
				public Void call(Row row) {
						String time = DateUtil.getCurrentDateTime("yyyyMMddHHmm");
						String kk = Constant.LAG_PLAY + "#ALL#ALL#ALL#ALL#" + time;
						logger.info("维度中包含所有ALL的key：" + kk + "    卡顿播放次数：" + row.getLong(0));
						hhm.getValue().put(kk, row.getLong(0));
						return null;
				}
			}).collect();
